    ...
    <div class="modal-dialog modal-dialog-scrollable" role="document">
        <div class="modal-content">
            <div class="modal-header">
                <h4 class="modal-title" id="myModalLabel1">تعديل اعلان</h4>
                <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                    <span aria-hidden="true">&times;</span>
                </button>
            </div>
            <div class="modal-body">
                <form class="form" action="{{ route('banners.update',$info->id) }}" method="POST" enctype="multipart/form-data">
                    @csrf
                    <div class="form-body">
                        ...
                        <div class="row">
                                ...
                                <div class="form-label-group">
                                    <select class="form-control" aria-invalid="false" name="region_id">
                                        ...
                                        @foreach ($regions as $reion)
                                            <option value="{{ $reion->id }}" {{ old('region_id',$info->region_id) == $reion->id ? 'selected' : '' }}>{{ $reion->name }}</option>
                                        @endforeach
                                    </select>
                                    @error('region_id')
                                    <span class="text-danger" id="basic-default-name-error" class="error">
                            ...
                            <div class="col-12">
                                <div class="form-label-group">
                                    <select class="form-control" aria-invalid="false" name="add_id">
                                        <option value="" disabled selected>-- اختر الاعلان --</option>
                                        @foreach ($adds as $add)
                                            <option value="{{ $add->id }}" {{ old('add_id',$info->add_id) == $add->id ? 'selected' : '' }}>{{ $add->title }}</option>
                                        @endforeach
                                    </select>
                                    @error('service_type')
                                    <span class="text-danger" id="basic-default-name-error" class="error">
